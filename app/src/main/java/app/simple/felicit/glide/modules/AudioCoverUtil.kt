...
fun ImageView.loadFromUri(context: Context, uri: Uri) {
    GlideApp.with(context).asBitmap().load(UriCoverModel(context, uri)).into(this)
}
...
fun ImageView.loadFromFileDescriptor(context: Context, uri: Uri) {
    GlideApp.with(context).asBitmap().load(DescriptorCoverModel(context, uri)).into(this)
}
 ...
 */
fun ImageView.loadFromPath(context: Context, path: String) {
    GlideApp.with(context).asBitmap().load(AudioCoverModel(path)).into(this)
}
 ...
 */
fun ImageView.loadArtistImage(context: Context, artistName: String) {
    GlideApp.with(context).asBitmap().load(ArtistImage(artistName, false)).into(this)
}